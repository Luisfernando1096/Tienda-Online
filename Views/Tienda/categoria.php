...
$arrProductos = $data['productos'];
$currentCategory = $data['infoCategoria']['categoria'];
?>
<br><br><br>
<hr>
<!-- Product -->
					...
					<div class="pagination-buttons">
						...
						<?php if ($data['pagina'] != $data['total_paginas']) { ?>
							<a href="<?= base_url() ?>/tienda/categoria/<?= $data['infoCategoria']['idcategoria'] . '/' . $data['infoCategoria']['ruta'] . '/' . $nextPagina ?>"
								class="pagination-button">
								Siguiente <i class="fas fa-chevron-right"></i></a>
						<?php } ?>
					</div>
				<?php } ?>
			</div>
						...
						<div class="flex flex-wrap p-t-4 m-r--5">
							...
							if (count($data['categorias']) > 0) {
								foreach ($data['categorias'] as $categoria) {
									// Verificar si la categoría es la actual
									$isActive = ($categoria['idcategoria'] == $data['infoCategoria']['idcategoria']) ? 'active' : '';
							?>
									<a href="<?= base_url() ?>/tienda/categoria/<?= $categoria['idcategoria'] . '/' . $categoria['ruta'] ?>" class="category-item flex-c-m stext-107 cl6 size-301 bor7 p-lr-15 hov-tag1 trans-04 m-r-5 m-b-5 <?= $isActive ?>">
...
<style>
	...
	.category-item.active {
		...
		/* Color de fondo para la categoría activa */
		color: white;
		/* Color del texto para la categoría activa */
		border: 1px solid #fcb388;
		/* Borde para la categoría activa */
	}
	...
	.current-category {
		...
		/* Sombra más ligera */
		display: flex;
		/* Usar flex para alinear elementos si es necesario */
		align-items: center;
		/* Centrar verticalmente */
	}

	/* Agregar un icono si deseas */
